...
- For every row with a Q2 locked forecast, compare it with the refreshed Q2 forecast using `abs(latest - locked) / abs(locked)`. When the difference exceeds 40%, write both values, the full calculation, and a cumulative-口径 warning in `备注` for manual review.
- Do not treat the `report_rc` Q2 label as an automatically valid single-quarter forecast: it can contain half-year cumulative or annual values. AG uses each institution's latest 2026 annual forecast in the 120 days before the Q2 actual announcement, takes the cross-institution median, then multiplies it by the company's historical Q2 profit share. Compare that value with the 8/7 lock snapshot; when the difference exceeds 40%, write the calculation in `备注`. If the recent annual-consensus sample is unavailable, AG falls back to the 8/7 lock; without a lock, use `2026Q1 actual × 2025 Q2/Q1` as an explicitly labelled seasonal fallback.
